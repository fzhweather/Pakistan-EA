...
# -*- coding: utf-8 -*-
"""
Created on Sat Jul 15 16:13:51 2023
...
# In[0]
import SSTforced_olr as SC
import SSTforced_t as SSTC
import zg_sst_cesm as SSTZ
import zg_sst_cesm_lower as SSTZl
...
loc = [[0.84, 0.74, 0.012, 0.32],[0.84, 0.53, 0.012, 0.25],[0.84, 0.36, 0.012, 0.28],[0.903, 0.448, 0.0085, 0.22],[0.335, 0.028, 0.0085, 0.32],[0.903, 0.028, 0.0085, 0.32]]
data = [SSTC.corr1,SC.corr1]
datap = [SSTC.corrp1,SC.corrp1]
level = [SSTC.levels,SC.levels]
regd = [-SSTZ.reg,-SSTZl.reg]
lev =[np.arange(-27,27.1,3),np.arange(-1.8,1.8001,0.2),np.arange(-0.9,0.9001,0.1)]
for i in range(3):
    ax.append(fig.add_axes([x1[i],yy[i],dx[i],dy[i]],projection = proj))
tit1 = ['a','b','c','d']
tit2 = ['Temperature','OLR','H200: ENSO-forced pattern','H850: ENSO-forced pattern']
...
for i in range(3):
    ...
    if i <= 1:
        ...
        poly1 = plt.Polygon(SSTC.xy1,edgecolor='red',linestyle='--',fc="none", lw=1.3, alpha=1,transform=ccrs.PlateCarree(),zorder=7)
        ax[i].add_patch(poly1)
        poly2 = plt.Polygon(SSTC.xy2,edgecolor='b',linestyle='--',fc="none", lw=1.3, alpha=1,transform=ccrs.PlateCarree(),zorder=7)
    ...
    if i > 1:
        ...
        poly1 = plt.Polygon(SSTC.xy1,edgecolor='red',linestyle='--',fc="none", lw=1, alpha=1,transform=ccrs.PlateCarree(),zorder=7)
        ax[i].add_patch(poly1)
        poly2 = plt.Polygon(SSTC.xy2,edgecolor='b',linestyle='--',fc="none", lw=1, alpha=1,transform=ccrs.PlateCarree(),zorder=7)
        ax[i].add_patch(poly2)
        position1=fig.add_axes(loc[i])#位置[左,下,长度,宽度]
        cbar=plt.colorbar(cb,cax=position1,orientation='vertical',ticks=np.arange(-24,24.001,12),
                         aspect=20,shrink=0.2,pad=0.06)
        cbar.ax.tick_params(length=1.8,width=0.4,pad=1.5)
